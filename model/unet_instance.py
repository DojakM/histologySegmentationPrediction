...
from model.model_components import UnetConv, UnetUp, UnetSPT, Context, Localization
...
class RTUnet(UnetSuper):
        def __init__(self, len_test_set, hparams, input_channels, is_deconv=True,
            ...
            self.input = input_channels
            filters = [32, 64, 128, 256]
            self.conv1 = UnetConv(self.in_channels, filters[0], 256, gpus=on_gpu, dropout_val=kwargs["dropout_val"])
            self.conv2 = UnetConv(filters[0], filters[1], 128,  gpus=on_gpu, dropout_val=kwargs["dropout_val"])
            self.conv3 = UnetConv(filters[1], filters[2], 64,  gpus=on_gpu, dropout_val=kwargs["dropout_val"])
